...
export type HeadingContentBlockInputProps = React.HTMLProps<HTMLTextAreaElement> & {
    index: number
    text: string
...
export const HeadingContentBlockInput: React.FC<HeadingContentBlockInputProps> = ({
    index,
    text,
    updateContentBlock,
    removeContentBlock,
    ...
    return (
        ...
        >
            <TextareaAutosize
                ...
                value={text}
                onChange={(event) => updateContentBlock(index, {type: 'HEADING', text: event.target.value})}
                className={clsx(
                    'bg-[transparent] appearance-none overflow-scroll scrollbar-hide outline-none resize-none w-full',
                    'font-interTight font-semibold text-primaryText text-2xl placeholder-secondaryText'
                )}
                minRows={1}
            />
            <Button
